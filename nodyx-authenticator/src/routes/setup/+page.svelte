	...
	onMount(async () => {
		...
		if (hub && token) {
			...
			try {
				...
				if (!info.authenticator) throw new Error('Cette communauté ne supporte pas encore Nodyx Signet.')
				hubName = info.name
				step = 'passphrase'
			} catch (e: unknown) {
	...
	{#if step === 'welcome'}
		...
		<div class="w-full max-w-sm flex flex-col items-center gap-8">
			<div class="flex flex-col items-center gap-3">
				...
				<h1 class="text-2xl font-bold tracking-tight">Nodyx Signet</h1>
				<p class="text-center text-sm leading-relaxed" style="color: var(--color-text-muted)">
					Connectez-vous à votre communauté Nodyx<br/>
			...
			<div class="w-full rounded-xl p-4 flex flex-col gap-3 text-sm" style="background: var(--color-surface); border: 1px solid var(--color-border)">
				<div class="flex items-start gap-3">
					...
					<span>Entrez l'adresse de votre communauté Nodyx</span>
				</div>
				<div class="flex items-start gap-3">
					<span style="color: var(--color-accent)">②</span>
	...
	{:else if step === 'hub'}
		...
		<div class="w-full max-w-sm flex flex-col gap-6">
			<div class="flex flex-col gap-1">
				<h2 class="text-xl font-bold">Votre communauté Nodyx</h2>
				<p class="text-sm" style="color: var(--color-text-muted)">
					C'est l'adresse du site Nodyx où vous avez votre compte.
				</p>
			</div>
			...
			<div class="flex flex-col gap-3">
				...
				<div class="flex flex-col gap-1.5">
					...
					<input
						...
						placeholder="Token généré dans vos paramètres Nodyx"
						class="w-full px-4 py-3 rounded-xl text-sm outline-none font-mono"
						style="background: var(--color-surface); border: 1px solid var(--color-border); color: var(--color-text)"
					/>
					<p class="text-xs" style="color: var(--color-text-muted)">
						Sur votre communauté Nodyx → Paramètres → Nodyx Signet → "+ Générer"
					</p>
				</div>
				{/if}
				<div class="flex flex-col gap-1.5">
