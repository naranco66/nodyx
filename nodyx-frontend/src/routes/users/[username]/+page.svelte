<script lang="ts">
	...
	// Ouvre (ou retrouve) une conversation DM avec l'utilisateur du profil et
	// redirige vers /dm/<conversationId>. Si la création échoue, on retombe
	// sur /dm pour ne pas laisser l'user dans le vide.
	let dmLoading = $state(false)
	async function openDm() {
		if (!me || !token || isOwnProfile || dmLoading) return
		dmLoading = true
		try {
