...
> Passwordless authentication PWA for [Nexus](../README.md) instances.

Nexus Signet is a Progressive Web App (PWA) that acts as a hardware security key for your browser. It generates an **ECDSA P-256 keypair** per device, stores it encrypted (AES-GCM) in IndexedDB, and signs login challenges from any Nexus instance — no password ever leaves your device.
...
```
1. Admin generates an enrollment token in Settings → Nexus Signet
...
Login flow:
6. User clicks "Connexion sans mot de passe" on the Nexus login page
...
The Nexus settings page generates a QR code encoding:
...
```bash
cd nexus-authenticator
npm install
npm run dev       # port 5174
```

Requires a running `nexus-core` instance with `SIGNET_URL=https://signet.nexusnode.app` set in `.env` (or `http://localhost:5174` for local dev).
...
Hosted separately from the main Nexus instance — `signet.nexusnode.app` is the official shared instance. Each Nexus community can optionally self-host their own Signet.

---

## API endpoints used (nexus-core `/api/auth/`)
...
├── nexus-authenticator/  ← Nexus Signet PWA (this folder)
