# SPEC 010 — NODYX GLOBAL SEARCH (THE MESH INDEX)
...
Nodyx Global Search is an inter-instance search protocol. It allows a user on instance A to find relevant content located on instances B, C, or Z, without ever going through a centralized search engine (Google/Bing).

The directory (Nodyx-Directory) evolves to become an **Index Relay**. Each Nodyx instance becomes a knowledge contributor that "seeds" the network with its discoveries.

---

## 🧠 2. WHY: The Impact

- **Breaking Silos:** Discord locks away knowledge. Nodyx frees it and connects it.
- **Organic SEO:** Creating a mesh of links (Mesh Links) so dense that AI engines (Perplexity/Ollama) will consider Nodyx the #1 community source of truth.
...
Unlike Google which "pulls" the web, Nodyx uses Push mode:
...
- **Ranking:** Results are ranked by:
  ...
  - Instance "reputation" (NodyxPoints)

### C. Security & Privacy

- **Zero Tracking:** The directory knows what is searched, but not who searches.
- **Opt-in:** Each instance admin decides whether to participate in the global index (`NODYX_GLOBAL_INDEXING=true`).
...
- **The "Trending Mesh":** A 3D visualization (or map) of topics currently "burning" across the entire Nodyx network. The more a topic is discussed, the brighter it glows on the map.
...
- **Search-to-Vocal:** Search not only returns text results, but also tells you if there's a "Cinema" Voice Channel currently live on that topic. "Looking for info on Rust? There's a live conference on the Dev-Nodyx instance, click to enter."
- **The "Teleportation Button":** In search results, a button that lets you instantly join a third-party instance with your current Nodyx identity (decentralized Single Sign-On).
