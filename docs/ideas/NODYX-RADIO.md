...
Ces métadonnées sont publiées dans le **directory nodyx.org** avec le profil de l'instance.
...
Les auditeurs trouvent via nodyx.org → arrivent dans l'instance → s'inscrivent
...
- `RadioDirectory.svelte` — liste des stations depuis nodyx.org/api/directory/radio
...
2026 : Un ancien animateur installe Nodyx sur un Raspberry Pi 4.
       ...
       Déclare sa station dans le directory nodyx.org.
...
Répartition :
  ...
  20% → nodyx.org (maintenance serveurs, développement Nodyx)
```
...
Et pour nodyx.org, c'est le modèle qui rend l'infrastructure **auto-suffisante**.
