<script lang="ts">
	...
	const stats = $derived([
		...
		{ label: 'Jours sur Nexus', value: daysSince.toLocaleString('fr-FR'), icon: '📅' },
	])
	...
	const title = $derived(`${profile.display_name || profile.username} — Nexus`)
	const description = $derived(
		profile.bio ? profile.bio.slice(0, 160) : `Profil de ${profile.display_name || profile.username} sur Nexus.`
	)
